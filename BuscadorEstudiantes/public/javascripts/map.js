    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            felipeMarker = new google.maps.Marker({
                ...
                icon: "/images/Estudiantes8to/"+data[0].photos[0]
            });
    ...
    data.forEach(d => {
        ...
        if (d.matricula != sessionStorage.getItem("matricula")){
            ...
            $("#usuariosLinea").append("<div onclick=\"IniciarChatCon(\'"+d.matricula+"\');\" id='"+d.matricula+"' style='height: 30px;border-bottom: 1px solid #d7dadc;cursor: pointer;'>"+
                                        ...
                                        "  <img src='/images/Estudiantes8to/"+d.img+"' style='width: 28px;border-radius: 50%;'> "+
                                        "  </div> "+
...
socket.on('puntos', (data) => {
    $("#puntosACompartir").html("");
    data.forEach(d => {
        //si hay uno repetido no lo agregue
        if (d.matricula != sessionStorage.getItem("matricula")){
            //onclick=\"eliminarChat(\''+uuidChat+'\');\" 
            $("#puntosACompartir").append("<div onclick=\"Marcar(\'"+d.matricula+"\');\" id='"+d.matricula+"' style='height: 30px;border-bottom: 1px solid #d7dadc;cursor: pointer;'>"+
                                        "  <div style='float:left'>"+
                                        "  <input type='checkbox' text='"+d.titulo+"' style='width: 28px;border-radius: 50%;'> "+
                                        "  </div> "+
                                        "  <div style='float: left;padding-left: 5px;line-height: 25px;'>"+d.nombre+"</div> "+
                                        "  <div style='clear:both'></div> "+
                                        "  </div>");
        ...
        success: function (data) {
            data.forEach(p => {
                ...
                if (p.type == "Estudiante") {
                    p.photos[0] = "/images/Estudiantes8to/" + p.photos[0];
                    icon = p.photos[0];
                }
                createMarker(p, icon);
            });
...
function createMarcador(item) {
    ...
    var contentString = '<div id="content">' +
        '<div id="siteNotice">' +
        '</div>' +
        '<h1 id="firstHeading" class="firstHeading">' + item.titulo + '</h1>' +
        '<div id="bodyContent">' +
