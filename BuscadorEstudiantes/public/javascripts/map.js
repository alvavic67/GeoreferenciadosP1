...
function consultaMatricula(matricula, dialog){
    ...
    $.ajax({ 
        ...
        data: {matricula: matricula},
        success: function(data){
            $("#marcadoresLinea").html("");
            data.forEach(m => {
                m.marcadores.lng = m.marcadores.coordinates[0];
                m.marcadores.lat = m.marcadores.coordinates[1];
                getAndCreateMarkers(m)
                $("#marcadoresLinea").append("<div onclick=\"IniciarChatCon(\'"+m.matricula+"\');\" id='"+m.matricula+"' style='height: 30px;border-bottom: 1px solid #d7dadc;cursor: pointer;'>"+
                                                "  <div style='float:left'>"+
                                                "  </div> "+
                                                "  <div style='float: left;padding-left: 5px;line-height: 25px;'>"+m.descripcion+"</div> "+
...
///////////////aqui se agrega el marcador al mapa y se manda para 
///////////////agregar a mongo
socket.on("agregar marcador",(usuario, marcador)=>{
    ...
    $.ajax({
        type: "POST",
        url: "/insertMarkers",
        data: {name: usuario, descripcion: marcador.titulo, lng: marcador.latlng.lng, lat: marcador.latlng.lat},
        success: function(data){
            console.log(data);
        },
        dataType: "json"
...
function getAndCreateMarkers(item) {
    var marker = new google.maps.Marker({
        map: map,
        position: item.marcadores,
        title: item.descripcion
    });
    console.log("Adentro del método: "+item.marcadores.coordinates)
    var contentString = '<div id="content">' +
        '<div id="siteNotice">' +
        '</div>' +
        '<h1 id="firstHeading" class="firstHeading">' + item.descripcion+ '</h1>' +
        '<div id="bodyContent">' +
        '</div>' +
        '</div>';
