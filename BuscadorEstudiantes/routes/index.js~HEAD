...
router.post("/getMarcadores", (req, res)=> {

    let query = { matricula: {'$regex': req.body.matricula} };

    mongoCliente.connect(url, function(err,db){
    if (err) throw err;

    db.collection("marcadores").find(query).toArray(function(err,result){
...
router.post("/insertMarcadores", (req, res)=> {
    let query ={
        marcadores:{
            type: "Point",
            coordinates: [parseFloat(req.body.lng), parseFloat(req.body.lat)]
        },
        matricula: req.body.name,
    ...
    mongoCliente.connect(url, (err, db)=> {
        if (err) throw err;
        db.collection("marcadores").insertOne(query, function(err, result){
            if (err) throw err;
            res.json(result);
            db.close();
        });
